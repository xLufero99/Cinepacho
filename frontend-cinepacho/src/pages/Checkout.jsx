...
console.log('👤 ID del usuario:', user?._id || user?.id);
  ...
  const [usingFreeTicket, setUsingFreeTicket] = useState(false);
...
const handleCompletePurchase = async () => {
  try {
    ...
    const purchaseUserId = customerUserId || (user?._id || user?.id);
    ...
    if (!purchaseUserId) {
      setModal({
        open: true,
        title: t('checkout.paymentFailedTitle'),
        message: 'No se pudo identificar al usuario que realizará la compra.',
        type: 'error'
      });
      return;
    }
    ...
    selectedSeats.forEach((seatId) => {
      ...
      const price = isPreferential 
        ? selectedShowtime?.precioPreferencial 
        : selectedShowtime?.precioGeneral;
    ...
    console.log('✅ Compra creada exitosamente!', { compraId: compra.id, estado: compra.estado });
    
    // 2. Procesar el pago
    console.log('💳 Procesando pago para compra:', compra.id);
    ...
    setModal({
      ...
      message: `${t('checkout.paymentSuccessMessage', { id: compra.id.slice(-6) })}\n${t('snacks.points')}: ${points}`,
      type: 'success',
      redirectTo: isEmployeeFlow ? '/employee/assistedBooking' : '/success'
    });
    ...
    releaseSeats();
    // Detectar error de inventario y simplificar mensaje
        ...
        <Container>
          <div className="text-center py-20">
            ...
            <p className="text-on-surface-variant mb-6">
              {t('checkout.selectMovieAndSeats')}
            </p>
            <Button onClick={() => navigate('/')}>{t('checkout.browseMovies')}</Button>
          </div>
        </Container>
      </MainLayout>
    );
          ...
          <div className="flex-grow min-h-0 overflow-y-auto pb-32">
              ...
              <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-lg flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-body text-xs text-on-surface-variant">
                    {t('checkout.ticketsConfirmedSubtitle')}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className={`mb-6 pb-6 ${borderBottomLight}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="font-headline text-lg text-on-surface font-medium">
                    {selectedMovie?.nombre || t('checkout.selectedMovie')}
                  </p>
                  {selectedShowtime && (
                    <p className="font-body text-sm text-on-surface-variant">
                      {selectedShowtime.fecha} • {selectedShowtime.hora}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <p className="font-headline text-lg text-on-surface font-medium">
                  {selectedHeadquarters?.nombre || t('checkout.selectedCinema')}
                </p>
            ...
            <div className={`mb-6 pb-6 ${borderBottomLight}`}>
              <p className="font-label text-xs text-on-surface-variant uppercase tracking-widest mb-3">
                {t('checkout.selectedSeats', { count: seatsCount })}
              </p>
              <div className="flex flex-wrap gap-2">
                {selectedSeats.map((seat) => (
                  <span key={seat} className={seatBadge}>
                    {seat}
                  </span>
                ))}
              </div>
            ...
            <div className={`space-y-3 mb-6 pb-6 ${borderBottomLight}`}>
              {isTicketsPaid ? (
                <>
                  ...
                  {hasSnackSelection && (
                    <PriceRow label={t('checkout.snacks')} value={snackTotal} />
                  )}
                </>
              ) : (
                <>
            ...
            <div className={`mb-6 pb-6 ${borderBottomLight}`}>
              ...
              {/* Free Ticket Redemption - One Click */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-label text-sm font-bold text-primary">
                          {t('loyalty.freeTicketAvailable')}
                        </p>
                        <p className="font-body text-xs text-on-surface-variant">
                          {availableFreeTickets.length} {t('loyalty.freeTicket')} {t('loyalty.available')}
                        </p>
                      </div>
                    </div>
                    <Button
            ...
            <div className={`mb-8 ${borderBottomLight}`}>
              <div className="flex justify-between items-baseline">
                <span className="font-headline text-xl text-on-surface-variant">
                  {isTicketsPaid ? t('checkout.amountDue') : t('checkout.total')}
                </span>
                <span className="font-headline text-3xl text-primary font-bold">
                  ${isTicketsPaid ? snackTotal.toLocaleString() : total.toLocaleString()}
                </span>
              </div>
              {isTicketsPaid && (
                <p className="font-body text-xs text-on-surface-variant text-right mt-1">
                  {t('checkout.ticketsAlreadyPaid', { amount: seatTotal.toLocaleString() })}
                </p>
              )}
            </div>
            ...
            <div className={`${cardBase} bg-surface-container-low p-6 md:p-8`}>
              <div className="space-y-3">
                {bookingStatus !== 'completed' ? (
                  ...
                  <div className="w-full py-4 text-lg text-center bg-surface-container-high rounded-full font-label font-bold text-on-surface-variant">
                    {t('checkout.bookingComplete')}
                  </div>
                )}
